...
import pt.pxinxas.graviball.game.entity.King;
...
public class CampaignKingLeagueScene extends BaseLeagueScene {
    ...
    public CampaignKingLeagueScene(int leagueLevel) {
        super(leagueLevel);
    }
    ...
    private void createOpponents(int leagueLevel) {
        ...
        King king = new King();
        king.setStatus(PlayerStatus.STOPPED);
        king.setScale(0.7f);
        this.opponents.add(king);
    }
    ...
    protected void onManagedUpdate(float pSecondsElapsed) {
        if (this.getStatus() == LeagueStatus.READY) {
            if (this.currentRound == null) {
                System.gc();
                KingRoundScene roundScene = new KingRoundScene(this, this.opponents);
